...
init.data.2species.discrete <- function(data=NA,M1=NA,M2=NA,inits=NA){
  ...
  for(i in 1:M1){
    if(sum(y1[i,,])>0){#if captured
      ...
      if(length(trapcaps)>1){
        s1.init[i,] <- colMeans(X[trapcaps,])
      }else{
        s1.init[i,] <- X[trapcaps,]
      }
    }
  }
  ...
  for(i in 1:M2){
    ...
    if(InSS[s2.cell.init[i]]==0){#not in SS, move to nearest cell
