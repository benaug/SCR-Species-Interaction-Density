sim.SCR.2species.discrete.interact <-
  function(D.beta01=NA,D.beta11=NA,rsf.beta1=NA,lambda.detect1=NA,sigma1=NA,
           D.cov1=NA,rsf.cov1=NA,
           D.beta02=NA,D.beta12=NA,beta.interact=NA,rsf.beta2=NA,lambda.detect2=NA,sigma2=NA,
           D.cov2=NA,rsf.cov2=NA,
           K=NA,X=X,xlim=NA,ylim=NA,res=NA,InSS=NA,seed=NA){
    ...
    #RSF Model, sps 1
    rsf1 <- exp(rsf.beta1*rsf.cov1)
    #get BVN availability and use distributions
    avail.dist1 <- use.dist1 <- matrix(NA,N1,n.cells)
    for(i in 1:N1){
      avail.dist1[i,] <- getAvail(s=s1[i,1:2],sigma=sigma1,res=res,x.vals=x.vals,
                                  y.vals=y.vals,n.cells.x=n.cells.x,n.cells.y=n.cells.y)
      use.dist1[i,] <- rsf1*avail.dist1[i,]
      use.dist1[i,] <- use.dist1[i,]/sum(use.dist1[i,])
    }
    
    #Density Model, sps 2
    D2.intercept <- exp(D.beta02)*cellArea
    use.dist1.sum <- colSums(use.dist1)
    D.term <- 100*(use.dist1.sum/cellArea)
    h <- exp(beta.interact*D.term)
    plot(h~D.term,xlab="Sps1 D",ylim=c(0,1))
    ...
    #plot sps2 over sps1
    image(x.vals,y.vals,matrix(use.dist1.sum,length(x.vals),length(y.vals)),
          main="Summed Species 1 Site Use Probs + Both Species ACs",xlab="X",ylab="Y",col=cols1)
    points(X,pch=4)
    points(s1[,1],s1[,2],col="darkred",pch=16)
    points(s2[,1],s2[,2],col="goldenrod",pch=16)
    
    #RSF Model, sps 2
    rsf2 <- exp(rsf.beta2*rsf.cov2)
    #get BVN availability and use distributions
    avail.dist2 <- use.dist2 <- matrix(NA,N2,n.cells)
    for(i in 1:N2){
      avail.dist2[i,] <- getAvail(s=s2[i,1:2],sigma=sigma2,res=res,x.vals=x.vals,
                                  y.vals=y.vals,n.cells.x=n.cells.x,n.cells.y=n.cells.y)
      use.dist2[i,] <- rsf2*avail.dist2[i,]
      use.dist2[i,] <- use.dist2[i,]/sum(use.dist2[i,])
    }
    ...
    lam1 <- lambda.detect1*use.dist1
    p1 <- 1 - exp(-lam1)
    y1 <- array(NA,dim=c(N1,J,K))
    for(i in 1:N1){
      for(j in 1:J){
        for(k in 1:K){
          y1[i,j,k] <- rbinom(1,1,p1[i,trap.to.cell[j]])
        }
      }
    }
    
    #simulate detection, sps 2
    lam2 <- lambda.detect2*use.dist2
    p2 <- 1 - exp(-lam2)
    y2 <- array(NA,dim=c(N2,J,K))
    for(i in 1:N2){
      for(j in 1:J){
        for(k in 1:K){
          y2[i,j,k] <- rbinom(1,1,p2[i,trap.to.cell[j]])
        }
      }
    }
    
    tmp <- colSums(use.dist1)
    image(x.vals,y.vals,matrix(tmp/sum(tmp),length(x.vals),length(y.vals)),
          main="Sps 1: Summed Site Use Probs + Spatial Recaps",xlab="X",ylab="Y",col=cols1)
    ...
    tmp <- colSums(use.dist2)
    image(x.vals,y.vals,matrix(tmp/sum(tmp),length(x.vals),length(y.vals)),
          main="Sps 2: Summed Site Use Probs + Spatial Recaps",xlab="X",ylab="Y",col=cols1)
    grid(n.cells.x,n.cells.y,lwd=1,lty=1,col="grey80")
    detected2 <- which(rowSums(y2)>0)
    points(s2[,1],s2[,2],pch=16,col="grey30",cex=1.25)
    points(X,pch=4)
    ...
    constants <- list(D.cov1=D.cov1,rsf.cov1=rsf.cov1,
                      D.cov2=D.cov2,rsf.cov2=rsf.cov2,
                      X=X,J=J,K=K,xlim=xlim,ylim=ylim,dSS=dSS,res=res,cells=cells,x.vals=x.vals,y.vals=y.vals,
                      trap.to.cell=trap.to.cell,InSS=InSS,n.cells=n.cells,n.cells.x=n.cells.x,n.cells.y=n.cells.y)
    truth <- list(lambda1=lambda1,lambda1.cell=lambda1.cell,rsf1=rsf1,N1=N1,s1=s1,s1.cell=s1.cell,
                  n1=n1,use.dist1=use.dist1,avail.dist1=avail.dist1,
                  lambda2=lambda2,lambda2.cell=lambda2.cell,rsf2=rsf2,N2=N2,s2=s2,s2.cell=s2.cell,
                  n2=n2,use.dist2=use.dist2,avail.dist2=avail.dist2)
