sSamplerDcovRSF1 <- nimbleFunction(
  # name = 'sampler_RW',
  ...
  run = function() {
    if(model$z1[i]==0){#propose from prior
      ...
      model$calculate(s1.nodes)
      copy(from = model, to = mvSaved, row = 1, nodes = s1.nodes, logProb = TRUE)
    }else{#MH
      ...
      if(inbox){
        ...
        use.dist1.sum.proposed <- use.dist1.sum.initial - model$use.dist1[i,] #subtract initial use.dist1[i,]
        
        #update proposed s
        model$s1.cell[i] <<- cells[trunc(s.cand[1]/res)+1,trunc(s.cand[2]/res)+1]
        lp_proposed_s1 <- model$calculate(s1.nodes)
        use.dist1.sum.proposed <- use.dist1.sum.proposed + model$use.dist1[i,] #add proposed use.dist1[i,] (updated in calculate above)
...
sSamplerDcovRSF2 <- nimbleFunction(
  # name = 'sampler_RW',
  contains = sampler_BASE,
  setup = function(model, mvSaved, target, control) {
    i <- control$i
  ...
  run = function() {
    if(model$z2[i]==0){#propose from prior
      ...
      if(inbox){
        ...
        model$s2.cell[i] <<- cells[trunc(s.cand[1]/res)+1,trunc(s.cand[2]/res)+1]
